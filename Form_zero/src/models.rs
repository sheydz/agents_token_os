...
pub fn assemble_runtime_text(
    ...
    // Only segment content becomes runtime head text. Batch objects are in-memory scheduler state,
    // and patch stays as side metadata unless some other path explicitly materializes it.
    ...
    let open_text_len = open_live_segment
        ...
        .unwrap_or_default();
    let mut combined =
        String::with_capacity(built_all_text.len() + process_text.len() + open_text_len + 48);
...
fn monitor_trigger_entry_from_value(value: &Value, slot_name: &str) -> Option<MonitorTriggerEntry> {
    ...
    Some(MonitorTriggerEntry {
            ...
            .unwrap_or(DeliveryAction::SegmentBoundaryDeliver.as_str())
        {
            ...
            "output_end_deliver" => DeliveryAction::OutputEndDeliver,
