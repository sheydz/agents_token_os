...
async fn append_ephemeral_mailbox_batch_to_live(
    ...
    let live_update = async {
        ...
        open_live.apply_delta(
            content,
            Some("mailbox_message".to_string()),
            Some("whitespace".to_string()),
            patch.clone(),
        )?;
...
impl RuntimeEngine {
    ...
    async fn handle_send_message_completed(
        ...
        self.queue_dispatcher
            .release_for_process(sender.process.id)
            .await;
        Ok(())
    }
    ...
    async fn judge_delivery_decision(
        ...
        let messages = vec![
            ...
            ProviderMessage::system(
                "For hardware_message, prefer mailbox delivery semantics over clone execution semantics.",
            ),
    ...
    async fn run_hardware_operation(
        ...
        let rendered_args = summarize_action_args(&step.operation_args);
        let transport = step.transport.as_deref().unwrap_or("unspecified");

        let value = match step.operation_name.as_str() {
            HARDWARE_OP_BOARD_INFO => json!({
                "operation": step.operation_name,
                "device_selector": step.device_selector,
                "transport": transport,
                "host_process_id": host.process.id.to_string(),
                "status": "ok",
            }),
            HARDWARE_OP_DEVICE_CAPABILITIES => json!({
                "operation": step.operation_name,
                "device_selector": step.device_selector,
                "transport": transport,
            ...
            HARDWARE_OP_GPIO_READ => json!({
                "operation": step.operation_name,
                "device_selector": step.device_selector,
                "transport": transport,
                "value": 0,
                "args": step.operation_args,
                "status": "ok",
            }),
            HARDWARE_OP_GPIO_WRITE => json!({
                "operation": step.operation_name,
                "device_selector": step.device_selector,
                "transport": transport,
                "applied": true,
            ...
            HARDWARE_OP_SERIAL_QUERY => json!({
                "operation": step.operation_name,
                "device_selector": step.device_selector,
                "transport": transport,
                "response": format!("query accepted {}", rendered_args),
                "status": "ok",
            }),
            HARDWARE_OP_SERIAL_WRITE => json!({
                "operation": step.operation_name,
                "device_selector": step.device_selector,
                "transport": transport,
                "written": true,
            ...
            HARDWARE_OP_PROBE_READ_MEMORY => json!({
                "operation": step.operation_name,
                "device_selector": step.device_selector,
                "transport": transport,
            ...
            HARDWARE_OP_FLASH_FIRMWARE => json!({
                "operation": step.operation_name,
                "device_selector": step.device_selector,
                "transport": transport,
                "flashed": true,
            ...
            _ => unreachable!("registry check should reject unsupported hardware operation"),
        };
...
mod tests {
    ...
    async fn append_ephemeral_mailbox_batch_to_live_updates_ephemeral_context() {
        ...
        assert_eq!(segment.segment_kind, "mailbox_message");
        ...
        assert_eq!(live_segment.segment_kind, "mailbox_message");
