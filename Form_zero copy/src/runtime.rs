...
use crate::{
    ...
    models::{
        ...
        monitor_trigger_entries_from_value, with_runtime_state, DeliveryAction, MessageKind,
        MessageSourceKind, MonitorTriggerEntry, MonitorTriggerEvent, OwnerKind,
        ProcessInstanceRow, ProcessRuntimeBinding, ProgramProcessBindingRow, QueueBatchInfo,
        QueueEnvelope, ResultTarget, RuntimeHead, SegmentRow, StepKind, TaskStep,
    },
    ...
    tools::{
        ...
        ToolRegistry, ToolRunRegistry, TOOL_APPEND_OUTPUT_REF, TOOL_CREATE_PROGRAM,
...
const DELIVERY_ACTION_ORDER: [DeliveryAction; 3] = [
...
const PROVIDER_STREAM_SEGMENT_KIND: &str = "token_stream";
// const PROCESS_BODY_SEGMENT_KIND: &str = "process_body";
// const SEND_MESSAGE_TRACE_SEGMENT_KIND: &str = "send_message_trace";
// const MAILBOX_MESSAGE_SEGMENT_KIND: &str = "mailbox_message";
// const EPHEMERAL_STREAM_SEGMENT_KIND: &str = "token_stream";
...
// fn sequence_actions_from_signal(signal: &RuntimeSignal) -> Vec<SequenceEngineAction> {
    match signal.kind {
        ...
        RuntimeSignalKind::ForceBoundary | RuntimeSignalKind::SendMessageCompleted => {
...
async fn append_process_segment_to_live_bus(
    ...
    if content.is_empty() {
        bail!("cannot append empty content to live bus for process {}", process.id);
    }
...
impl QueueDispatcher {
    ...
    async fn accept_envelope(&self, envelope: QueueEnvelope) {
        ...
        match final_delivery_action {
            ...
            DeliveryAction::SegmentBoundaryDeliver | DeliveryAction::OutputEndDeliver => {
    ...
    // async fn mark_release_pending(&self, process_id: Uuid, cause: impl Into<String>) {
    //     let mut inner = self.mailboxes.inner.lock().await;
    //     let mailbox = inner.entry(process_id).or_insert_with(ProcessMailbox::new);
    //     mailbox.mark_release_pending(cause);
    // }

    // async fn request_release_for_process(&self, process_id: Uuid, cause: impl Into<String>) {
    //     let cause = cause.into();
    //     let has_open_live = self.live_bus_registry.get(process_id).await.is_some();
        let batches = {
            let mut inner = self.mailboxes.inner.lock().await;
            let Some(mailbox) = inner.get_mut(&process_id) else {
            // let mailbox = inner.entry(process_id).or_insert_with(ProcessMailbox::new);
            // if has_open_live || mailbox.lanes.is_empty() {
            //     mailbox.mark_release_pending(cause);
                return;
            };
            let batches = mailbox.drain_release_batches();
            // mailbox.clear_pending_release();
            if mailbox.is_empty() {
        ...
        for batch in batches {
            self.deliver_batch(batch).await;
    //         Box::pin(self.deliver_batch(batch)).await;
    //     }
    // }

    // async fn apply_sequence_actions(&self, actions: Vec<SequenceEngineAction>) -> Result<()> {
    //     for action in actions {
    //         match action {
    //             SequenceEngineAction::SealCurrentSegment { process_id } => {
    //                 let target = match self.db.get_process_runtime_binding(process_id).await {
    //                     Ok(binding) => binding,
    //                     Err(_) => continue,
    //                 };
    ...
    //                 .await?;
    //             }
    //             SequenceEngineAction::ReleaseMailbox { process_id } => {
    //                 self.request_release_for_process(process_id, "sequence_release")
    //                     .await;
    //             }
    //             SequenceEngineAction::MarkMailboxReleasePending { process_id, cause } => {
    //                 self.mark_release_pending(process_id, cause).await;
    //             }
    //             SequenceEngineAction::OpenSegment { process_id, segment_kind } => {
    //                 let _ = (process_id, segment_kind);
    //             }
    //             SequenceEngineAction::AppendToCurrent {
    //                 process_id,
    //                 segment_kind,
    //                 content,
    //                 patch,
    //             } => {
    //                 let _ = (process_id, segment_kind, content, patch);
    //             }
    //         }
        }
        // Ok(())
    ...
    async fn deliver_batch(&self, batch: MailboxBatch) {
        ...
        //     .sequence_engine
        //     .ingest(RuntimeSignal {
        //         kind: RuntimeSignalKind::ReceiveStarted,
        ...
        //         segment_kind: Some(MAILBOX_MESSAGE_SEGMENT_KIND.to_string()),
        ...
        // match self.append_mailbox_batch_to_live(&target, &content, patch).await {
            Ok(segment) => {
                let _ = self
                    .sequence_engine
                    .ingest(RuntimeSignal {
                ...
                let actions = self
                    .sequence_engine
                    .ingest(RuntimeSignal {
                        kind: RuntimeSignalKind::ReceiveCompleted,
                        observed_process_id: target.process.id,
                        observed_stream_id: None,
                        observed_slot: target
                            .binding
                    ...
                    .await;
                let _ = self.apply_sequence_actions(actions).await;
    ...
    async fn force_interrupt_for_process(&self, target: &ProcessRuntimeBinding) -> Result<()> {
        ...
        let actions = self
            .sequence_engine
            .ingest(RuntimeSignal {
                kind: RuntimeSignalKind::ForceInterrupted,
                observed_process_id: target.process.id,
                observed_stream_id: None,
                observed_slot: target
                    .binding
            ...
            .await;
        self.apply_sequence_actions(actions).await?;
        Ok(())
    }
    ...
    async fn deliver_via_async_clone(&self, mut envelope: QueueEnvelope) -> Result<()> {
        ...
        if delivery_action == DeliveryAction::EphemeralAsyncClone {
            ...
            .await?;
            if envelope.message_kind == MessageKind::TaskMessage {
                ...
                metadata = merge_patch(
                    metadata,
                    Some(json!({
                        "bounce_hint": "ephemeral_async_clone currently only executes task_message without persistence"
                    })),
                );
                envelope.metadata = metadata;
                self.bounce_message(envelope).await?;
            }
            return Ok(());
        }
    ...
    async fn bounce_message(&self, envelope: QueueEnvelope) -> Result<()> {
        ...
        let target_process_id = bounced.target_process_id;
        self.global_message_queue.enqueue(bounced)?;
        self.request_release_for_process(target_process_id, "bounce_message_send")
            .await;
        Ok(())
    }
    ...
    async fn record_forced_event_results(
        ...
        for envelope in envelopes.iter().filter(|envelope| {
            ...
            let sender_binding = self
                ...
                .get_process_runtime_binding(envelope.sender_process_id)
                .await?;
            let _ = append_process_body_to_live_bus(
                &self.db,
                &self.live_bus_registry,
                &self.runtime_head_registry,
                &self.sequence_engine,
...
fn delivery_action_from_str(value: &str) -> Option<DeliveryAction> {
    match value {
        ...
        "output_end_deliver" => Some(DeliveryAction::OutputEndDeliver),
...
fn render_send_message_trace_started(envelope: &QueueEnvelope) -> String {
    ...
    if let Some(content_text) = envelope.content.as_deref() {
        ...
        content.push_str("]\n");
    } else if !envelope.task_sequence.is_empty() {
...
impl RuntimeEngine {
    ...
    async fn handle_send_message_request(
        ...
        let _process_stream_permit =
            self.acquire_process_stream_slot(sender_binding.process.id).await?;
        ...
        let envelope = self
            ...
            .await?;
        self.handle_send_message_started(&sender_binding, &envelope)
            .await?;
        self.global_message_queue.enqueue(envelope.clone())?;
        self.handle_send_message_completed(&sender_binding, &target_binding, &envelope)
            .await?;
        Ok(envelope)
    }

    async fn handle_send_message_started(
        &self,
        ...
        let actions = self
            .sequence_engine
            .ingest(RuntimeSignal {
                kind: RuntimeSignalKind::SendStarted,
            ...
            .await;
        self.queue_dispatcher.apply_sequence_actions(actions).await?;
        ...
        let _ = append_process_segment_to_live_bus(
            ...
            SEND_MESSAGE_TRACE_SEGMENT_KIND,
            &trace_content,
            Some("whitespace"),
            Some(json!({
    ...
    async fn handle_send_message_completed(
        ...
        let trace_segment = append_process_segment_to_live_bus(
            ...
            Some(json!({
                ...
                "release_target_process_id": target.process.id.to_string(),
            })),
            Some(sender.process.id),
            Some("send_message"),
            None,
        ...
        let signal = RuntimeSignal {
            kind: RuntimeSignalKind::SendCompleted,
            observed_process_id: sender.process.id,
            observed_stream_id: None,
            observed_slot: sender
                .binding
        ...
        let actions = self.sequence_engine.ingest(signal).await;
    ...
    async fn enqueue_slot_monitor_triggers(
        &self,
        sender: &ProcessRuntimeBinding,
    ) -> Result<()> {
        ...
        let Some(slot_state) = program.plan_state_json.slots.get(&binding.program_slot_name) else {
            return Ok(());
        };
            ...
            .filter(|trigger| trigger.trigger_event == MonitorTriggerEvent::SendMessageCompleted)
        {
            ...
            let envelope = self
                .prepare_message_envelope(
                    ...
                    None,
                    match trigger.emitted_message_kind {
                        MessageKind::NormalMessage => trigger
                            .content_template
                            .clone()
                            .or_else(|| Some(trigger.text.clone())),
                        MessageKind::TaskMessage => None,
                    },
                    None,
            ...
            self.queue_dispatcher
                .request_release_for_process(target.process.id, "monitor_trigger_send")
                .await;
        }
    ...
    async fn acquire_process_stream_slot(&self, process_id: Uuid) -> Result<OwnedSemaphorePermit> {
        let permit = self.process_stream_gates.acquire(process_id).await?;
        while self.live_bus_registry.get(process_id).await.is_some() {
                ...
                let live_update = async {
                    let mut open_live = match self.live_bus_registry.get(process_id).await {
                        ...
                        None => OpenLiveSegment::new(
                            ...
                            .await?,
                            PROCESS_BODY_SEGMENT_KIND.to_string(),
                    ...
                    open_live.apply_delta(
                        content_delta,
                        Some(PROCESS_BODY_SEGMENT_KIND.to_string()),
                        Some(PROVIDER_STREAM_TOKENIZER.to_string()),
                        process_body_patch(
                            "token_stream",
                            provider_stream_patch(tool_name, tool_run_id, process_id, None),
                ...
                let live_update = async {
                    let mut open_live = match self.ephemeral_live_bus_registry.get(stream_id).await
                    {
                        ...
                        None => OpenLiveSegment::new(
                            stream_id,
                            1,
                            EPHEMERAL_STREAM_SEGMENT_KIND.to_string(),
                            Some(PROVIDER_STREAM_TOKENIZER.to_string()),
                    ...
                    open_live.apply_delta(
                        content_delta,
                        Some(EPHEMERAL_STREAM_SEGMENT_KIND.to_string()),
                        Some(PROVIDER_STREAM_TOKENIZER.to_string()),
                        provider_stream_patch(
                            tool_name,
                            tool_run_id,
    ...
    async fn complete_provider_stream(
        ...
        let actions = self
            .sequence_engine
            .ingest(RuntimeSignal {
                kind: RuntimeSignalKind::OutputCompleted,
            ...
            .await;
        if matches!(provider_stream_target.live_owner, LiveOwnerRef::Process(_)) {
            self.queue_dispatcher.apply_sequence_actions(actions).await?;
        }
        Ok(())
    }
    ...
    async fn prepare_message_envelope(
        ...
        let (final_priority, final_delivery_action, bounce_hint) = self
    ...
    async fn judge_delivery_decision(
        ...
        let default = (base_priority, requested_delivery_action, None);
        ...
        let messages = vec![
            ...
            ProviderMessage::system(
                "Allowed actions: interrupt_deliver, persistent_async_clone, segment_boundary_deliver, output_end_deliver, ephemeral_async_clone, bounce_with_hint, ignore, blacklist_sender.",
            ),
    ...
    async fn send_remote_action_result(
        ...
        self.global_message_queue.enqueue(envelope)?;
        self.queue_dispatcher
            .request_release_for_process(target.process.id, "remote_action_result_send")
            .await;
        Ok(())
    }
    ...
    async fn run_action_core(
        ...
        if tool_requires_guide(tool_name, tool_args) && extract_tool_guide(tool_args).is_none() {
        ...
        match tool_name {
            ...
            _ => bail!("unsupported tool_name {}", tool_name),
        }
    }
    ...
    async fn tool_spawn_real_process(
        ...
        let prefix_suffix_definite_id = optional_uuid(tool_args, "prefix_suffix_definite_id")
            .unwrap_or(host.process.prefix_suffix_definite_id);
        let status = optional_string(tool_args, "status").unwrap_or_else(|| "running".to_string());
        ...
        if !program
            .plan_state_json
            .slots
            .contains_key(&program_slot_name)
        {
            let upstream_group =
    ...
    async fn tool_spawn_branch_process(
        ...
        let segment = if materialize_to_host {
            Some(
                append_process_body_to_live_bus(
                    &self.db,
                    &self.live_bus_registry,
                    &self.runtime_head_registry,
                    &self.sequence_engine,
    ...
    async fn tool_make_plan(
        ...
        let binding = host
            .binding
            .as_ref()
            .ok_or_else(|| anyhow!("make_plan requires host process bound to a program slot"))?;
        let guide = extract_tool_guide(tool_args);
    ...
    async fn tool_append_output_ref(
        ...
        let segment = append_process_body_to_live_bus(
            &self.db,
            &self.live_bus_registry,
            &self.runtime_head_registry,
            &self.sequence_engine,
    ...
    async fn generate_text_with_provider(
        ...
        if !tool_requires_provider(tool_name, &Value::Null) && self.provider.is_none() {
            return Ok(provider_fallback_text(tool_name, guide, Some(task)));
        }
        ...
        let Some(provider) = self.provider.as_ref() else {
            let output = provider_fallback_text(tool_name, guide, Some(task));
            self.append_provider_chunk(provider_stream_target, tool_name, tool_run_id, &output)
                .await?;
            if matches!(provider_stream_target.live_owner, LiveOwnerRef::Process(_)) {
                self.complete_provider_stream(host, provider_stream_target, tool_name, tool_run_id)
                    .await?;
            } else if let LiveOwnerRef::Ephemeral(_) = provider_stream_target.live_owner {
                self.complete_provider_stream(host, provider_stream_target, tool_name, tool_run_id)
                    .await?;
            }
            self.tool_run_registry
        ...
        match provider_handle.await {
            ...
            Ok(Err(error)) => {
                if output.is_empty() {
                    output = provider_fallback_text(tool_name, guide, Some(task));
                }
...
fn monitor_triggers_from_args(
    ...
    triggers.extend(
        crate::models::step_entries_from_value(tool_args.get("plan_loop"), slot_name, StepKind::Plan)
            .into_iter()
